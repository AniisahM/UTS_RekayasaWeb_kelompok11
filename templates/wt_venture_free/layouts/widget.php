...
// Set default panel
if ($panel == '' && in_array($widget->position, array('slideshow', 'top-a', 'top-b', 'top-c', 'top-d', 'bottom-a', 'bottom-b', 'bottom-c', 'bottom-d', 'bottom-e','bottom-f', 'main-top', 'main-bottom', 'sidebar-a', 'sidebar-b'))) {
    $panel = $this['config']->get("panel_default.{$widget->position}.panel", '');
}
// Set panel for specific positions
else if (in_array($widget->position, array('toolbar-r' ,'toolbar-l', 'above-footer','footer', 'offcanvas'))) {
	$panel = 'uk-panel';
}
...
if ($widget->menu) {
	...
	} else if (in_array($widget->position, array('toolbar-l', 'toolbar-r','above-footer', 'footer'))) {
		$renderer = 'subnav';
